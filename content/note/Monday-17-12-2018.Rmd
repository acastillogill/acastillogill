---
title: "Fixing the Namibian mystery"
subtitle: "Namibia's country code is NA, guess what that causes?"
author: Ángela Castillo-Gill
date: '2018-12-17'
slug: fixing-the-namibian-mystery
categories:
  - Journal
tags: 
  - Learning
draft: FALSE
summary: "Namibia's country code is NA, guess what that causes?"
output:
 blogdown::html_page:
  fig_caption: true
editor_options: 
...
downloads_internet_plot_out<- outliers_gone%>%
  arrange(desc(n))%>%
  ggplot(aes(users, n, label=name, color=sub_region))+
  #Add light opacity
  geom_point(alpha=0.5)+
  #Add nice commas x axis
  scale_x_continuous(labels=comma_format())+
...
downloads_nested %>% 
  ...
  mutate(region=fct_reorder(sub_region,median_n))%>%
  ggplot(aes(x=sub_region, y=median_n, fill=region))+
  geom_col()+
  expand_limits(y=0)+
  coord_flip()+
  scale_y_continuous(labels=comma_format())+
...
model_performance%>%
  ...
  select(sub_region, adj.r.squared)%>%
  kable(caption="The model describing R downloads as a function of internet users per country and the HDI has significant coefficients for Asia and Europe.")
```
