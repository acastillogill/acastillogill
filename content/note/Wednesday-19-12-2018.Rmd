...
subtitle: "Cross-validating with MAP functions"
author: Ángela Castillo-Gill
date: '2018-12-19'
slug: model-cross-validation
categories:
  - Journal
tags: 
  - Learning
draft: FALSE
summary: "Cross-validating with MAP functions"
output:
 blogdown::html_page:
  fig_caption: true
editor_options: 
...
## Cross validating the models with the MAP function

```{r creating-train-test-data}
set.seed(123)
library(rsample)
#Setting seed
...
```{r creating-train-and-validate-data-from-each-fold}
cv_data <- cv_split%>%
...
```{r creating-models-with-cross-validated-data}
...
```{r extract-actual-and-predicted-values}
...
So on average, our models deviate by `r mean(cv_eval_lm$validate_mae)` downloads per country. 
