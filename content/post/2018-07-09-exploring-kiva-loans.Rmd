...
grid.arrange(
  ...
  OC_borrower_genders,
  nrow = 1,
  top = textGrob("Borrower genders by continent"),gp=gpar(fontsize=20,font=3)
)
```
...
```{r repayment-interval-and-continent, fig.cap="Most popular repayment styles by continent"}
...
grid.arrange(AS_repayment_interval, AF_repayment_interval, SA_repayment_interval, OC_repayment_interval,nrow=1)

```

In Asia and Oceania, the most popular repayment interval is irregular while in Africa and South America it's monthly. 

```{r sectors-and-continents, fig.cap="Most popular uses of loans by continent"}
...
grid.arrange(AS_sector, AF_sector, SA_sector, OC_sector,nrow=1)
```
