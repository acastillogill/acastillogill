...
```{r}
#Get a glimpse of kiva_locations
glimpse(kiva_locations)
```
It seems that this set is dedicated to showing the specific region but has 2,772. It doesn't match the first dataset. I assume this dataset is related to the Kaggle contest related to the dataset. 

```{r}
#Get a glimpse of kiva_themes
glimpse(kiva_themes)
```
It seems that this dataset could be linking the first one with a particular larger `theme`, as defined by Kiva. 

Finally, the last one. 

```{r}
#Get a glimpse of kiva_locations_themes
glimpse(kiva_locations_themes)
```

It seems that this dataset contains more georeferenced information. Since I'm interested in **learning what variables contribute to the final amount lended without specific geographic information, (country is enough)**, I'm going to stick with the first dataset, `kiva_loans`. I'm tempted to jump into a regression of sorts, but first, let's have a closer look at the data to understand the variables and decide which ones are most helpful. 
...
Finally, we still would like to understand what countries are most represented and which ones, less so. Let's have a look. 
...
na.omit(kiva_loans)%>%
 ...
 theme(axis.title.y=element_blank())+
  labs(title="Top 20 countries that request Kiva Loans",
       x="Country count",
       y="")
...
ggplot(kiva_loans,aes(x=loan_usd))+
  ...
  annotate("text",
           ...
           color="red")
```
The median loan in US dollars is 4.19. The distribution is multimodal and right-skewed with its biggest peak around $1 and its second around $4. 
