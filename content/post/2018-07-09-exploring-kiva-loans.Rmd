...
pop_iso_loans %>%
  top_n(10) %>%
  ggplot(aes(reorder(x = country_code, n_per_cap), y = n_per_cap)) +
  geom_col(position = "dodge",
  color = "purple",
  fill = "purple",
  alpha=0.6) +
  coord_flip() +
  theme_minimal() +
  labs(title = "Top 10 countries requesting Kiva loans per capita",
  x = "",
  y = "Number of loans per capita")
```

Let's see how the top 10 changes when it comes to internet users. I used the numbers published by the International Telecommunications Union available [here](https://www.itu.int/en/ITU-D/Statistics/Pages/stat/default.aspx).
...
ggplot(kiva_loans, aes(x = loan_usd)) +
  geom_density(color="purple",alpha=0.3,fill="purple") +
  theme_classic() +
...
ggplot(kiva_loans,
       aes(x=lender_count))+
  geom_histogram(color="purple",alpha=0.3,fill="purple")+
   theme_classic()+
