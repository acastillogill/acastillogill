...
The complete dataset was a zip file with size 232.7 MB containing four files: `kiva_loans.csv`, `kiva_mpi_region_locations.csv`,`loan_theme_ids.csv`, and `loan_themes_by_region.csv`. After I looked at the contents, I chose to work with the first one.
...
The dataset was large with 671,205 observations and 20 variables.
...
```{r repayment-interval-and-continent, fig.cap="In Asia and Oceania, the most popular repayment interval is irregular while in Africa and South America it's monthly.",fig.width=15}
...
```{r sectors-and-continents, fig.cap="In every continent, the three most popular sectors are agriculture, retail, and food.",fig.width=12}
...
```{r countries-plot, fig.cap="Philippines is the country that requests the mosts loans with over double as the second country Kenya `KE`. Other top countries are Cambodia `KH`, Pakistan `PK`, Peru `PE`, Colombia `CO`, Uganda `UG`, Tajikistan `TJ`, Ecuador `EC`, and Paraguay `PY`."}
...
Figure \@ref(fig:loan-usd-distribution), shows that the distribution of Kiva loans in US dollars is right skewed and multimodal with its biggest peak around $1 and its second around $4. 

```{r term_in_months_distribution, fig.cap="The loan term in months is a bimodal distribution with its first peak around 8 months and its second around 12, the median."}
ggplot(kiva_loans,
...
```{r lender_count, fig.cap="The lender count has a text book right-skewed distribution with a median of 12 lenders."}
ggplot(kiva_loans,
       aes(x=lender_count))+
  geom_histogram()+
   theme_classic()+
...
```{r giving_time, fig.cap="The median time between posting the loan and the loan being fully funded is about 9.2 days. It is a bimodal distribution with its first peak around 6 days and its second peak around 30 days."}
ggplot(kiva_loans,
...
```{r total_time, fig.cap="The total time between posting the loan and it being disbursed has a funky looking multimodal distribution. It seems to be cut off at 30 days with peaks at 7, 14, 21 and 30 days. Median time is 17.2 days."}
ggplot(kiva_loans,
