...
kiva_loans %>%
  ...
  x = "",
  y = "Number of loans")+
 geom_label(aes(label=n), nudge_x = -0.5)
```
...
```{r countries-plot-capita, fig.cap="Interesting to see how the ranking changed! Leading the charts is the island of Samoa, followed by Armenia. Countries that remain in the top 10 are Cambodia, Philippines, Kenia, and Tajikstan. Other new countries are Timor-Leste, Paraguay, Palestine, and Lebanon"}
pop_iso_loans %>%
  top_n(10) %>%
...
Let's see how the top 10 changes when it comes to internet users. I used the ranking based on numbers published by the International Telecommunications Union available [here](https://en.wikipedia.org/wiki/List_of_countries_by_number_of_Internet_users).
...
```{r countries-plot-users, fig.cap="Interesting to see how the ranking changed! Leading the charts is the island of Samoa, followed by Armenia. Countries that remain in the top 10 are Cambodia, Philippines, Kenia, and Tajikstan. Other new countries are Timor-Leste, Paraguay, Palestine, and Lebanon"}
users_iso_loans %>%
  top_n(10) %>%
