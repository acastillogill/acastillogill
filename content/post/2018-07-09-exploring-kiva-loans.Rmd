...
kiva_16 <- kiva_16%>%
  group_by(country_code)%>%
  mutate(country_lender_median = median(lender_count),
         nobs=n())%>%
  ungroup()%>%
  mutate(label_lender = paste(country_code, "\n", "Median = ",
                           round(country_lender_median, digits = 2),
                           "\n", "N = ", nobs),
         label=reorder(label_lender, -nobs))
  
  twee
#Make a ggplot with kiva_16, where the x axis is the lender count and the fill will be nobs. 
gg_kiva16 <- ggplot(kiva_16, aes(x=lender_count, fill=nobs))
#Create a histogram
gg_kiva16 + geom_histogram(position="identity", binwidth = 1, alpha=0.7, fill="#84628e")
#Create facets
gg_kiva16 + facet_wrap( ~ label, ncol = 4, scales="free")
#Add the median line
gg_kiva16 + geom_vline(aes(xintercept = country_lender_median), linetype="dotted")
#Add the theme
gg_kiva16 + theme_minimal() 
#Remove the axis
gg_kiva16 + theme(axis.text.x = element_blank())
#Add labels
gg_kiva16 + labs(x = "Lender count",
                 y = "Frenquency",
                 title = "How many lenders fund a single Kiva microloan per country?", 
                 subtitle="Top 16 borrowing countries included.")
```
