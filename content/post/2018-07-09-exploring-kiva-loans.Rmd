...
```{r countries-plot, fig.cap="Philippines requests the mosts loans with over double as the second country, Kenya `KE`. Other top countries in this ranking are Cambodia `KH`, Pakistan `PK`, Peru `PE`, Colombia `CO`, Uganda `UG`, Tajikistan `TJ`, Ecuador `EC`, and Paraguay `PY`."}
...
kiva_loans %>%
  ...
  x = "",
  y = "Number of loans")+
 geom_label(aes(label=n), nudge_y = 0.1)
```
...
# Questions that came from this analysis
