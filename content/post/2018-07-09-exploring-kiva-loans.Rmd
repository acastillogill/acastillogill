...
```{r continent-and-borrower-genders-plots, fig.cap="Four continents in descending order from the continent that requests the most loans (Asia). Notice Europe and North America do not appear. `mult_females` or `mult_males` means more than one female or male.",fig.width=10}
...
```{r repayment-interval-and-continent, fig.cap="In Asia and Oceania, the most popular repayment interval is irregular while in Africa and South America it's monthly.",fig.width=10}
...
```{r sectors-and-continents, fig.cap="In every continent, the three most popular sectors are agriculture, retail, and food.",fig.width=10}
