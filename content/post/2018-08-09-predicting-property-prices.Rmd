...
```{r flattenCorrMatrix function, include=FALSE}
#https://github.com/anthonypc/mulitpleLinearRegression/blob/master/iris-multiple-variable-regression-example.R
...
corMatrix <- rcorr(as.matrix(all_homes), type = "pearson")
...
anova_table <-  rownames_to_column(anova_table)

colnames(anova_table) <- c("Variable","DF","SumSq","MeanSq","FValue","PValue")

anova_table <- anova_table%>%
  dplyr::select(-DF)%>%
  filter(PValue<0.000000000005)%>%
  arrange(PValue)
...
```{r}
fit_21 <-  lm(SalePrice ~ OverallQual+
                LotFrontage+
                X2ndFlrSF+
                LotArea+
                ExterQual+
...
all_models <- dredge(fit_21)

all_models%>%
  arrange(AICc)%>%
  top_n(5)#%>%
  #dplyr::select(logLik, AICc, delta, weight)%>%
```
