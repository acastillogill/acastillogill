...
In addition to that, I downloaded the education index for all countries. The education index is a metric published by the United Nations and it corresponds to the average of the Mean Years of Schooling Index (MYSI) and the Expected Years of Schooling Index (EYSI). 
...
To further explore the relationship between countries that intense R users and education, I decided to do some regression analysis using the education index and the main independent variable.
...
- A one unit increase in the education index, in this case, one percentage point, is expected to increase R downloads per capita by `r (exp(summary(fit)$coef[2])-1)*100` percent.

- So for example, if the fictious country Republic of Ángela with an education index of 78 and downloads per capita of 1.1, increased its education index to 79, R downloads per capita would be expected to increase to 1.1*1.109=1.22.
...
fit%>%
  ...
  labs(title="The Education Index vs. R Downloads per Capita",
       subtitle="Black dots are real values and red dots are fitted values. \nFor every percentage point increase in the education index, \nthe geometric mean of R downloads per capita is expected to increase by 10.9%",
       x="Education index",
       y="R downloads per capita")+
  expand_limits(0,0)
```
