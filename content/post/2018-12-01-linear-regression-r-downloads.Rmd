...
- The geometric mean of R downloads per 1,000 capita is 0.00004917.

- A one unit increase in the education index, in this case, one percentage point, is expected to increase, on average, R downloads per 1,000 capita by `r summary(fit)$coef[2]*100`%.

- There is a 95% chance that the following interval contains the true value of the coefficient corresponding to the education index: [`r (summary(fit)$coef[2]-2*summary(fit)$coef[2,2])*100`% and `r (summary(fit)$coef[2]+2*summary(fit)$coef[2,2])*100`%.]

- So for example, if the fictitious country Republic of Ángela with an education index of 78 and downloads per 1,000 capita of 1.1, increased its education index to 79, R downloads per 1,000 capita would be expected to increase between a range of 1.1 * 1.0916 = 1.201 and 1.1 * 1.1153 = 1.227.
