...
```{r variable-importance-table, fig.cap="Overall quality is the most predictive variable and the one that originally had the most correlation with `SalePrice`, it's followed by basement quality and Total basement square feet."}
...
```{r final_plot, fig.cap="The tree with 10 nodes and 9 splits had the least lowest performance metric `RMSE`."}
...
The most influential variable for `SalePrice` was `OverallQual`. This variable "rates the overall material and finish of the house", values equal or above to 8 correspond to "very good", "excellent", and "very excellent". On one hand, for houses with a rating of 8 or above in `OverallQual`, the next most decisive variable is `TotalBsmtSF`, which is the "total square feet of basement area". If it's above 1,850, the house is classified depending on the total number of rooms. If it's below 1,850 then `GarageArea` will determine how much the house will sell for. 

On the other hand, houses with a rating below 8 for `OverallQual`, `Neighborhood_type` defined as median price of houses in that neighborhood. The variables that determine the `SalePrice` thereafter are `GrLivArea`: "above grade living area in square feet", and `OverallQual` again.
