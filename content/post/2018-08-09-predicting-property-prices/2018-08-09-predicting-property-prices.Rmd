...
ggplot(homes,
       aes(x=reorder(Neighborhood, SalePrice, FUN = sum), y=SalePrice)) +
  geom_col () +
  theme_minimal() +
  labs(title="Neighborhoods vs. SalePrice",
       x="Neighborhood")+
...
# Classification and Regression Trees (CART) time

In this post I am using the recursing and partitioning (RPART) algorithm also known as classification and regression trees (CART). It will be implemented with the `rpart` package in R. 
...
- What are the further improvements that the algorithm can make? 

These are further splits that reduce the homogeneity of the subgroups.
